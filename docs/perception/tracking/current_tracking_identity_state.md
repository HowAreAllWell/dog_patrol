...
- `BirthCandidateStore` 已作为 identity 层内部可单测 birth candidate storage helper 抽取，legacy matcher 把小目标新人 per-raw-track consecutive hit / last-seen frame 状态交给 helper 维护，并继续负责何时擦除 pending candidate。
...
因此，当前 identity 层接口、一部分 shadow / Phase 4/5 evidence route、update-policy 决策面、feature-bank read / appearance-cost 规则、reliable-geometry read / prediction / cost / gate 规则、assignment cost composition 规则、active assignment solving / acceptance 规则、inactive recovery selection 规则、raw continuity assignment decision 规则、birth / hidden candidate decision 规则、birth candidate storage / stability counter 规则、semantic id allocation counter / skip-existing-id 规则、assignment candidate/debug 构造规则、assignment application planning 规则、assignment apply mutation orchestration 规则、raw-to-semantic binding storage 规则、identity record store facade 规则、feature bank / reliable geometry mutation 规则、legacy identity record 类型边界，以及 record lifecycle mutation 规则已经迁移；legacy record 内部也已收敛出 feature bank / reliable geometry 子状态。但底层 identity state storage ownership、feature bank / reliable geometry ownership、完整 birth ownership 和部分 legacy 对照逻辑仍主要在 legacy matcher 内部。
...
26. 已将 Phase 6 birth candidate storage / small-new-person stability counter 抽取为内部 `BirthCandidateStore` helper，覆盖 per-raw-track consecutive hit increment、gap reset、raw-id independent counter、erase 和 clear；`LegacyIdentityMatcher` 仍负责 decision input adaptation、semantic-id allocation、debug row aggregation、Phase 5 flag adaptation 和完整 birth apply ownership。
27. 已将 Phase 6 assignment application planning 抽取为内部 `AssignmentApplicationPlan` helper，覆盖已决 assignment candidates 的 duplicate semantic collision fallback、accepted debug-row lookup、assignment cost / margin / stage 推导和 next raw-to-semantic map construction；`LegacyIdentityMatcher` 仍负责 update-policy evaluation、`UpsertIdentity`、occlusion protection、row reason defaults、`AgeAndPruneIdentities` 和实际 state mutation。
28. 已将 Phase 6 raw-to-semantic binding storage 抽取为内部 `RawSemanticBindingStore` helper，覆盖 previous snapshot、planned replacement、lookup、direct bind/erase/reset 和保持当前 raw-map 插入口径；`LegacyIdentityMatcher` 仍负责决定何时在 `Update`、Phase 4 handoff、pairwise assignment 和 Phase 5 accepted birth allocation 路径调用该 helper。
29. 已将 Phase 6 legacy identity record store facade 抽取为内部 `LegacyIdentityStore` helper，覆盖 semantic-id lookup、occupied-id 枚举、active/inactive person semantic id 枚举、frame-begin seen reset、age-one-frame、occlusion protection、single-blob carrier handoff missing 和 sorted snapshot projection；`LegacyIdentityMatcher` 不再直接协调这些 record container 操作，但仍持有 candidate collection、score/debug row aggregation、assignment application 编排和完整 birth ownership。
30. 已将 Phase 6 assignment candidate/debug 构造抽取为内部 `AssignmentCandidateBuilder` helper，覆盖 active assignment candidate score/gate evidence 到 `assign_candidate` rows 与 solver matrix 的适配、inactive recovery candidate decision 到 `inactive_recover_candidate` rows 的适配，以及 solver result selected / margin / accepted / pending row 回写；`LegacyIdentityMatcher` 仍持有 pairwise debug rows、实际 assignment application、raw-to-semantic mapping、semantic-id allocation call sites、birth decisions、Phase 4/5 apply paths 和 identity record storage ownership。
31. 已将 Phase 6 assignment application mutation orchestration 抽取为内部 `AssignmentApplicationExecutor` helper，覆盖按既有顺序迭代 planned applications、accepted row 的 update-policy reason 回写、feature / reliable-geometry mutation、merged/split/overlap 遮挡保护、rejected row default reason 填充，以及 planned raw-to-semantic replacement；`LegacyIdentityMatcher` 通过 `EvaluateUpdatePolicy` / `UpsertIdentity` seams 提供现有行为，仍持有 birth decisions、Phase 4/5 direct apply call sites 与完整 birth ownership。
...
4. feature update policy 决策面、feature-bank read/cost helper、reliable-geometry read/cost helper、assignment cost composition helper、active assignment solving helper、inactive recovery selection helper、raw-continuity decision helper、birth / hidden candidate decision helper、birth candidate storage helper、semantic id allocation helper、assignment application planning helper、assignment apply executor helper、raw-to-semantic binding storage helper、feature bank / reliable geometry mutation helper、legacy identity record 类型边界和 record lifecycle mutation helper 已独立，legacy record 内也已有 feature bank / reliable geometry 子状态；但长期特征管理、feature bank ownership、reliable geometry ownership 和完整 birth ownership 仍主要在 legacy matcher 内完成。
