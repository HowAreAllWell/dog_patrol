...
因此，当前 identity 层接口、一部分 shadow / Phase 4/5 evidence route、update-policy 决策面、feature-bank read / appearance-cost 规则、reliable-geometry read / prediction / cost / gate 规则、assignment cost composition 规则、active assignment solving / acceptance 规则、inactive recovery selection 规则、raw continuity assignment decision 规则、birth / hidden candidate decision 规则、birth candidate storage / stability counter 规则、semantic id allocation counter / skip-existing-id 规则、assignment candidate/debug 构造规则、assignment application planning 规则、raw-to-semantic binding storage 规则、identity record store facade 规则、feature bank / reliable geometry mutation 规则、legacy identity record 类型边界，以及 record lifecycle mutation 规则已经迁移；legacy record 内部也已收敛出 feature bank / reliable geometry 子状态。但底层 identity state storage ownership、feature bank / reliable geometry ownership、assignment apply 编排、完整 birth ownership 和部分 legacy 对照逻辑仍主要在 legacy matcher 内部。
...
4. feature update policy 决策面、feature-bank read/cost helper、reliable-geometry read/cost helper、assignment cost composition helper、active assignment solving helper、inactive recovery selection helper、raw-continuity decision helper、birth / hidden candidate decision helper、birth candidate storage helper、semantic id allocation helper、assignment application planning helper、raw-to-semantic binding storage helper、feature bank / reliable geometry mutation helper、legacy identity record 类型边界和 record lifecycle mutation helper 已独立，legacy record 内也已有 feature bank / reliable geometry 子状态；但长期特征管理、feature bank ownership、reliable geometry ownership 和完整 assignment / birth ownership 仍主要在 legacy matcher 内完成。
