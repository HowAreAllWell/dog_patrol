...
37. 已将 Phase 4 direct identity apply mutation mechanics 抽取为内部 `Phase4DirectApplyHelper`，覆盖 raw track lookup、target semantic id validation、feature extraction、app/geo/time/final cost、accepted Phase 4 score debug row、update policy、upsert、raw binding 和 accepted raw birth cleanup；四个 `LegacyIdentityMatcher` public Phase 4 apply methods、`Phase4HandoffCoordinator` 编排、single-blob carrier missing side effect、side-recovery force-geometry-update 和 public 输出语义保持不变。
