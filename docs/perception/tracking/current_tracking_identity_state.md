...
- `IdentityRuntimeStore` 已作为 identity 层内部可单测 record store facade 抽取，assignment engine adapter 把 semantic-id lookup、occupied-id / active-inactive person 枚举、frame-begin reset、occlusion protection、carrier handoff missing 和 sorted snapshot projection 委托给该 facade；lost / inactive aging 的 end-of-frame lifecycle 协调已移到 `IdentityRuntimeLifecycleCoordinator`。
...
- Phase 5 BirthManager 已固定为唯一运行时 birth / hidden candidate 路径；hidden / pending / accepted birth decision surface 由 `IdentityManager` Phase 5 helper 表达，accepted birth allocation 通过 `IdentityRuntimeMutationApplier` 应用，并以 `stage=phase5_birth_candidate` / `stage=phase5_new_semantic` 及 `new_birth_candidate_*` rows 记录。
...
11. 已加入 Phase 5 `NewBirthCandidate` shadow-only lifecycle rows：`new_birth_candidate_pending`、`new_birth_candidate_hidden`、`new_birth_candidate_allocated`，用于观察 birth / hidden / allocation 决策。
...
13. 已将 Phase 6 update-policy decision calculation 抽取为 `FeatureUpdatePolicy` helper；`IdentityAssignmentEngineAdapter` 继续负责可靠观测判定、overlap/global freeze 输入适配和 debug row 聚合。
...
39. 已将 lost / inactive aging lifecycle 的 end-of-frame 协调移到 `IdentityRuntimeLifecycleCoordinator`。`IdentityAssignmentEngineAdapter` 仍在原 `Update` 末尾时机调用该 runtime-owned helper，保持 missing-frame aging、occlusion protection countdown、snapshot projection、debug row timing 和 public identity state 语义不变。
...
3. 再逐步把 adapter 内剩余 assignment / lifecycle 编排迁移到目标 `IdentityManager` 状态机；
