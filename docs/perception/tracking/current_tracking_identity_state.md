...
- `MergedSingleBlobAssignmentDecision` 已作为 identity 层内部可单测 merged single-blob decision helper 抽取，legacy matcher 在 `kMerged` + one visible person 路径中继续负责 identity storage、feature/cost/gate/inactive candidate preparation、semantic id allocation 和 raw binding，helper 只决定 active candidate selection、continuity margin、best-appearance handoff eligibility、Phase 4 handoff 前的 continuity preference、inactive recovery fallback 与 new semantic id fallback。
...
- `Phase4DirectApplyHelper` 已作为 identity 层内部 Phase 4 direct apply mutation helper 抽取；`IdentityRuntimeMutationApplier` 在 identity runtime/coordinator 层调用该 helper，并接管 accepted Phase 4/5 runtime mutation。`LegacyIdentityMatcher` 不再公开 Phase 4/5 direct apply methods；single-blob carrier missing side effect、side-recovery force-geometry-update、Phase 5 accepted birth allocation row refresh、raw binding 和 birth cleanup 由 runtime applier 保持。
...
- `IdentityManager::Impl` 已持有 identity runtime state，集中包含 `LegacyIdentityStore`、`RawSemanticBindingStore`、`SemanticIdAllocator`、`BirthManager`、frame index、primary/bootstrap state、occlusion mode state 和 debug row buffers；`LegacyIdentityMatcher` 作为内部 adapter 在该 state 上运行。
