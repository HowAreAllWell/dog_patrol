...
4. feature update policy 决策面、feature-bank read/cost helper、reliable-geometry read/cost helper、assignment cost composition helper、active assignment input/solving helper、inactive recovery input/selection helper、raw-continuity decision helper、birth / hidden candidate decision helper、birth candidate storage helper、birth facade helper、semantic id allocation helper、assignment application planning helper、assignment apply executor helper、raw-to-semantic binding storage helper、feature bank / reliable geometry mutation helper、legacy identity record 类型边界和 record lifecycle mutation helper 已独立，legacy record 内也已有 feature bank / reliable geometry 子状态；但长期特征管理、feature bank ownership、reliable geometry ownership，以及 birth apply 后的 identity storage / primary / output ownership 仍主要在 legacy matcher 内完成。
