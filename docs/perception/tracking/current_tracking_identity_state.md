...
9. Phase 5 birth / hidden candidate 已有 shadow-only `NewBirthCandidate` lifecycle evidence，但还没有独立 `BirthManager`。当前 `ambiguous_recovery_pending`、`duplicate_split_hidden`、`skinny_partial_hidden`、`wide_fragment_hidden`、small stable new-person promotion 和 `new_semantic` allocation 仍由 `LegacyIdentityMatcher` 决策。
...
2. 先基于 Phase 5 `NewBirthCandidate` shadow lifecycle 验证 birth / hidden candidate 的 shadow equivalence；
