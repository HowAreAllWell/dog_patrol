...
- feature bank / reliable geometry 子状态 storage；
- `FeatureUpdatePolicy` 输入适配、`FeatureBankCost` feature-bank read 委托、`ReliableGeometryCost` reliable-geometry read 委托与 `FeatureGeometryUpdateState` mutation 委托；
...
4. feature update policy 决策面、feature-bank read/cost helper、reliable-geometry read/cost helper 和 feature bank / reliable geometry mutation helper 已独立，legacy record 内也已有 feature bank / reliable geometry 子状态；但长期特征管理、feature bank ownership 和 reliable geometry ownership 仍主要在 legacy matcher 内完成。
...
2. 在已抽取的 `FeatureUpdatePolicy` / `FeatureBankCost` / `ReliableGeometryCost` / `FeatureGeometryUpdateState` seam 和 legacy record 内嵌子状态基础上继续小步迁移 feature bank / reliable geometry ownership；
