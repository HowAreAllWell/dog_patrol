...
- `ActiveAssignmentInputCollector` 已作为 identity 层内部可单测 active assignment input collector 抽取，legacy matcher 把 unassigned-track / free-semantic-id filtering、per-track feature selection/index adaptation 和 score/gate evidence adaptation 委托给 helper，再把结果交给 `AssignmentCandidateBuilder` 和 `ActiveAssignmentSolver`。
- `InactiveRecoveryInputCollector` 已作为 identity 层内部可单测 inactive recovery input collector 抽取，legacy matcher 把 recovery-track filtering、free inactive semantic-id filtering、recoverable identity filtering、feature selection/index adaptation、recover threshold 和 missing identity gate evidence adaptation 委托给 helper，再把结果交给 `InactiveRecoverySolver` 和 `AssignmentCandidateBuilder`。
...
- `UnresolvedTrackFinalResolutionCoordinator` 已作为 identity 层内部可单测 final resolution helper 抽取，legacy matcher 在 raw continuity、active assignment、inactive recovery 后，把仍未分配的 visible person track 编排交给该 helper，覆盖 occlusion-suspect skip、selected margin-reject ambiguous recovery hold、merged side recovery、显式 rollback suppression、BirthManager input adaptation、pending erase 请求和 accepted birth assignment output。
...
- `MergedSingleBlobAssignmentDecision` 已作为 identity 层内部可单测 merged single-blob decision helper 抽取，legacy matcher 在 `kMerged` + one visible person 路径中继续负责 identity storage、feature/cost/gate/inactive candidate preparation、semantic id allocation 和 raw binding，helper 只决定 active candidate selection、continuity margin、best-appearance handoff eligibility、rollback-compatible continuity preference、inactive recovery fallback 与 new semantic id fallback。
...
- `Phase4DirectApplyHelper` 已作为 identity 层内部 Phase 4 direct apply mutation helper 抽取，legacy matcher 的四个 public Phase 4 apply methods 保持原签名和返回语义，并把 raw track lookup、semantic id validation、feature/cost/debug row、update policy、upsert、raw binding 和 birth cleanup 委托给 helper；single-blob carrier missing side effect 和 side-recovery force-geometry-update 仍由原路径保持。
...
- `sid.enable_phase5_birth_manager` 迁移 flag 默认开启；默认路径中 hidden / pending / accepted birth decision surface 由 `IdentityManager` Phase 5 helper 表达，accepted birth allocation 通过 legacy state seam 应用，并以 `stage=phase5_birth_candidate` / `stage=phase5_new_semantic` 及 `new_birth_candidate_*` rows 记录；显式设为 `false` 时回退 legacy-compatible `birth_candidate` / `new_semantic` 路径。
...
12. Phase 5 birth migration flag `sid.enable_phase5_birth_manager` 已默认开启。默认路径迁移 hidden / pending / accepted birth decision surface 到 `IdentityManager` Phase 5 path，同时保留 legacy score/debug rows 对照；显式 `false` 回退 legacy `birth_candidate` / `new_semantic`。
...
27. 已将 Phase 6 birth / hidden candidate facade 抽取为内部 `BirthManager` helper，覆盖 ambiguous recovery / duplicate split / morphology hide / small-new-person stability 的 decision input adaptation、pending store 擦除时机、semantic id allocation call timing、birth debug row 构造，以及 `phase5_birth_candidate -> phase5_new_semantic` accepted allocation apply seam；`LegacyIdentityMatcher` 仍持有 birth apply 后的 identity storage / primary / output ownership 和 rollback 对照面。
...
32. 已将 Phase 6 active assignment input collection 抽取为内部 `ActiveAssignmentInputCollector` helper，覆盖 unassigned-track filtering、free semantic id filtering、feature row/index adaptation、missing identity / appearance gate evidence 与 accepted/pairwise-pending row compatibility 所需输入准备；`LegacyIdentityMatcher` 仍持有 score truth helpers、solver invocation、pairwise debug rows、assignment application、raw-to-semantic mapping、birth decisions、Phase 4/5 apply paths 和 identity record storage ownership。
33. 已将 Phase 6 assignment application mutation orchestration 抽取为内部 `AssignmentApplicationExecutor` helper，覆盖按既有顺序迭代 planned applications、accepted row 的 update-policy reason 回写、feature / reliable-geometry mutation、merged/split/overlap 遮挡保护、rejected row default reason 填充，以及 planned raw-to-semantic replacement；`LegacyIdentityMatcher` 通过 `EvaluateUpdatePolicy` / `UpsertIdentity` seams 提供现有行为，仍持有 birth decisions、Phase 4/5 direct apply call sites 与 birth apply 后的 identity storage / primary / output ownership。
34. 已将 Phase 6 merged single-blob assignment decision 抽取为内部 `MergedSingleBlobAssignmentDecision` helper，覆盖 continuity kept、best final-cost selection、close continuity candidate margin、best-appearance handoff eligibility、rollback flag preferring continuity、inactive recovery fallback 和 new semantic id fallback；`LegacyIdentityMatcher` 仍持有 identity storage、feature extraction、cost/gate 适配、inactive recovery candidate preparation、semantic id allocation、raw binding、Phase 4 apply methods、primary/output/CSV/UDP/overlay ownership。
35. 已将 Phase 6 inactive recovery input collection 抽取为内部 `InactiveRecoveryInputCollector` helper，覆盖 recovery-track filtering、free inactive semantic-id filtering、`CanRecoverInactiveIdentity` 过滤、strict / relaxed recover threshold 适配、missing identity gate evidence 和 accepted / selected row compatibility 所需输入准备；`LegacyIdentityMatcher` 仍持有 score truth helpers、solver invocation、recovery application、raw-to-semantic mapping、birth decisions、Phase 4/5 apply paths 和 identity record storage ownership。
36. 已将 Phase 6 unresolved-track final resolution 编排抽取为内部 `UnresolvedTrackFinalResolutionCoordinator` helper，覆盖 active / inactive assignment 后仍未分配 track 的 occlusion-suspect skip、ambiguous recovery pending、merged side recovery accepted / rollback suppression、duplicate split / morphology hide、小目标 pending、Phase 5 pending 和 accepted birth assignment output；`LegacyIdentityMatcher` 仍负责 score truth callbacks、BirthManager / SemanticIdAllocator call sites、debug row frame/mode projection、identity storage mutation、raw binding、Phase 4/5 direct apply、primary/output/CSV/UDP/overlay ownership。
37. 已将 Phase 4 direct identity apply mutation mechanics 抽取为内部 `Phase4DirectApplyHelper`，覆盖 raw track lookup、target semantic id validation、feature extraction、app/geo/time/final cost、accepted Phase 4 score debug row、update policy、upsert、raw binding 和 accepted raw birth cleanup；四个 `LegacyIdentityMatcher` public Phase 4 apply methods、`Phase4HandoffCoordinator` 编排、rollback flags、single-blob carrier missing side effect、side-recovery force-geometry-update 和 public 输出语义保持不变。
