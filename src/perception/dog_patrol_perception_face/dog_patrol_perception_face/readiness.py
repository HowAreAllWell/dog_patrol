"""ROS-independent sequencing for face capability readiness."""
...
from .preflight import ERROR, NOT_READY, READY, FacePreflightOutcome


class FaceReadinessController:
    """Run one preflight at a time and publish only its current STARTUP result."""

    def __init__(
        self,
        preflight: Callable[[], FacePreflightOutcome],
        publish: Callable[[FacePreflightOutcome], None],
        *,
        startup_state: int = 0,
    ) -> None:
        ...
        self._worker = threading.Thread(
            target=self._run,
            name="dog-patrol-face-readiness",
            daemon=True,
        )
        self._worker.start()
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                outcome = FacePreflightOutcome(ERROR, _exception_detail(exc))
