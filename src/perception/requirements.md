...
| voice | `integrating` | `dog_patrol_perception_voice` 的 R818/Vosk 核心、异步 evidence provider、只读 voice preflight/readiness、无人参与和最终现场验收 CLI、Prompt player、安装 helper 和受控配置；设备与 Vosk 模型由部署机提供 | preflight、安装产物、transient-local `voice` capability 接口、#37 自动验收入口和强制匹配部署资产及验收代码指纹的 #38 现场矩阵入口已接入；真人矩阵、来源归档、模型效果、音频播放和生产现场验收仍待完成 |
