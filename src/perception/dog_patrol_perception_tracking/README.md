...
- `target.handled_ignore_absence_sec`（总控通过 `MissionState.handled_target_id` 明确标记已处理后，该目标需要连续不可见多久才允许再次被选中；当前为 60 s，定位失败不进入该排除名单）
...
已验证的资产构建基线是 JetPack 6.2.1、CUDA 12.6、TensorRT 10.3 和
`MV-CU013-A0UC`。YOLO26n FP16 engine 必须在目标 Orin 本机构建，不假定可跨设备或跨
JetPack/TensorRT 版本复用；Python 工具的固定依赖见
`../../../assets/models/manifests/tracking_core_requirements.txt`。模型和 engine 本身不进入仓库。
...
ros2 run dog_patrol_perception_tracking dog_patrol_perception_tracking_node --ros-args \
  ...
  -p detector.runtime_path:='/path/to/dog_patrol/assets/models/engines/orin_jp621_trt_local/yolo26n_fp16_640.engine' \
  -p tracker.config_path:='/path/to/dog_patrol/install/dog_patrol_perception_tracking/share/dog_patrol_perception_tracking/config/bot_sort.yaml' \
...
```bash
/path/to/dog_patrol/src/perception/dog_patrol_perception_tracking/scripts/eval_tracker_core_round1.sh \
...
- 当前 tracking identity 实现状态：
  `../../../docs/perception/tracking/current_tracking_identity_state.md`
- 遮挡与 ID 问题的有效结论已收敛到
  `../../../docs/perception/tracking/tracking_algorithm_design_and_tuning.md`；历史录像和现场资产不随仓库迁入。
...
```bash
/path/to/dog_patrol/src/perception/dog_patrol_perception_tracking/scripts/bench_hik_mvs_camera.sh
```
...
`../../../docs/perception/tracking/issue80_hik_mvs_frame_contract_baseline.md`。

`#85` 已补充同源 Bayer、含人受控 detector/ReID/tracking 和完整 `balanced` live 30 FPS 证据，
见 `../../../docs/perception/tracking/issue85_bayer_preprocess_audit.md`。当前 config file、参数声明、live helper 和 Hik MVS
...
`../../../docs/perception/tracking/issue82_ffv1_offline_eval_baseline.md` 记录的 migration regression 输入，不是默认或 canonical 数据集。
...
输出结构：
- `/path/to/dog_patrol/data/eval_results/<run_name>_<timestamp>/`
- 每个测试集子目录（默认短名）：
  - `s01/`, `s02/`, ...（可通过参数关闭短名）
