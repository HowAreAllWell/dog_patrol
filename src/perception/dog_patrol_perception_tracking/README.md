...
未接入或未完成：
- identity 层仍主要由 `LegacyIdentityMatcher` 承担，尚未迁移为完整的新状态机；`FeatureUpdatePolicy` 决策 helper、`FeatureGeometryUpdateState` mutation helper、`FeatureBankCost` read/cost helper、`ReliableGeometryCost` reliable-geometry read/cost helper、`AssignmentCost` cost composition helper、`ActiveAssignmentSolver` active assignment helper、`ActiveAssignmentInputCollector` active assignment input collector helper、`InactiveRecoveryInputCollector` inactive recovery input collector helper、`InactiveRecoverySolver` inactive recovery helper、`RawContinuityDecision` raw continuity helper、`BirthCandidateDecision` birth / hidden candidate helper、`BirthCandidateStore` birth candidate storage helper、`BirthManager` birth / hidden candidate facade、`UnresolvedTrackFinalResolutionCoordinator` unresolved-track final resolution helper、`SemanticIdAllocator` semantic id allocation helper、`AssignmentCandidateBuilder` assignment candidate/debug helper、`MergedSingleBlobAssignmentDecision` merged single-blob assignment decision helper、`AssignmentApplicationPlan` assignment application planning helper、`AssignmentApplicationExecutor` assignment apply executor helper、`Phase4DirectApplyHelper` Phase 4 direct apply mutation helper、`RawSemanticBindingStore` raw-to-semantic binding helper、`LegacyIdentityStore` identity record store facade 与 identity record lifecycle helper 已抽取。legacy matcher 不再直接协调 record container 的 lookup / enumeration / lifecycle / snapshot，也不再直接构造 active / inactive assignment candidate debug rows，active assignment input collection、inactive recovery input collection、merged single-blob assignment decision、assignment apply mutation orchestration、birth decision / pending / Phase 5 allocation orchestration、active/inactive 后仍未分配 track 的 final resolution 编排，以及 Phase 4 direct identity apply mutation mechanics 也已委托给 helper；`sid.enable_phase5_birth_manager` 已默认开启且可显式设为 `false` 回退 legacy-compatible 路径，但 identity storage ownership、primary/output 仍保持 legacy 边界。
- 距离/2D 相对位置
- 控制逻辑
...
- Phase 5 birth / hidden candidate readiness：
  ...
  - `sid.enable_phase5_birth_manager=true` 默认启用时，`sid_scores.csv` 中 `stage=phase5_birth_candidate` 表示 Phase 5 birth proposal / hidden / pending decision，`stage=phase5_new_semantic`、`accepted=1` 表示 Phase 5 path 已正式分配 semantic id；显式设为 `false` 时仍使用 legacy `birth_candidate` / `new_semantic` rollback。
  - `phase3_shadow_state.csv` 中 `new_birth_candidate_pending|new_birth_candidate_hidden|new_birth_candidate_allocated` 为 Phase 5 lifecycle evidence；默认路径下 pending / allocation decision 由 `IdentityManager` Phase 5 path 产生，显式 rollback 时保持 rollback-compatible 观测。
