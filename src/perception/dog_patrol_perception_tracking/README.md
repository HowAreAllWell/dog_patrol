...
未接入或未完成：
- identity 层仍主要由 `LegacyIdentityMatcher` 承担，尚未迁移为完整的新状态机；`FeatureUpdatePolicy` 决策 helper、`FeatureGeometryUpdateState` mutation helper、`FeatureBankCost` read/cost helper、`ReliableGeometryCost` reliable-geometry read/cost helper、`AssignmentCost` cost composition helper、`ActiveAssignmentSolver` active assignment helper、`InactiveRecoverySolver` inactive recovery helper、`RawContinuityDecision` raw continuity helper、`BirthCandidateDecision` birth / hidden candidate helper、`BirthCandidateStore` birth candidate storage helper、`SemanticIdAllocator` semantic id allocation helper、`AssignmentCandidateBuilder` assignment candidate/debug helper、`AssignmentApplicationPlan` assignment application planning helper、`RawSemanticBindingStore` raw-to-semantic binding helper、`LegacyIdentityStore` identity record store facade 与 identity record lifecycle helper 已抽取。legacy matcher 不再直接协调 record container 的 lookup / enumeration / lifecycle / snapshot，也不再直接构造 active / inactive assignment candidate debug rows，但 assignment apply 编排和完整 birth ownership 仍在 legacy 内部。
- 距离/2D 相对位置
- 控制逻辑
