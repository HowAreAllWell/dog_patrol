...
class VisionDemoNode : public rclcpp::Node {
 ...
 private:
  ...
  void LogEffectiveConfig(const vision_demo_host::CameraIngest::Config &camera_cfg,
    ...
    RCLCPP_INFO(get_logger(),
                "startup_effective_config identity_assign active_max=%.3f recovery_max=%.3f raw_continuity_max=%.3f min_margin=%.3f stable_feature_frames=%d merged_requires_overlap=%s phase4_merged_split_handoff=%s phase4_merged_side_recovery=%s reid=%s reid_backend=%s reid_model=%s reid_input=%dx%d",
                ...
                BoolStr(identity_cfg.enable_phase4_merged_side_recovery), BoolStr(identity_cfg.reid_enable),
