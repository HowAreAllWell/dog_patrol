...
ReliableGeometryCost::State IdentityAssignmentEngineAdapter::ReliableGeometryState(const LegacyIdentityRecord &identity) const {
...
bool IdentityAssignmentEngineAdapter::PassesMissingIdentityGate(const Track &track, const LegacyIdentityRecord &identity,
...
bool IdentityAssignmentEngineAdapter::PassesMissingAppearanceGate(const LegacyIdentityRecord &identity, const float app_cost,
...
void IdentityAssignmentEngineAdapter::ComputeCosts(const Track &track, const LegacyIdentityRecord &identity, const std::vector<float> &feature,
...
float IdentityAssignmentEngineAdapter::AssignmentScore(const Track &track, const LegacyIdentityRecord &identity,
...
float IdentityAssignmentEngineAdapter::ActiveAssignmentMaxCost(const LegacyIdentityRecord &identity,
...
FeatureUpdatePolicy::Decision IdentityAssignmentEngineAdapter::EvaluateUpdatePolicy(
    const Track &track, const std::vector<Track> &tracks, const int self_idx, const LegacyIdentityRecord *identity,
...
void IdentityAssignmentEngineAdapter::UpdateIdentityObservation(LegacyIdentityRecord *identity, const Track &track,
                                                   const float assignment_cost,
                                                   const float assignment_margin) const {
  LegacyIdentityRecordLifecycle::ApplyObservation(track, runtime_state_->frame_index, assignment_cost, assignment_margin, identity);
}
...
bool IdentityAssignmentEngineAdapter::LooksLikeMergedSideReappearance(
    const Track &candidate, const LegacyIdentityRecord &identity, const std::vector<Track> &tracks, const int candidate_idx,
...
float IdentityAssignmentEngineAdapter::RecoverThresholdForSemantic(const LegacyIdentityRecord &identity) const {
...
bool IdentityAssignmentEngineAdapter::CanRecoverInactiveIdentity(const LegacyIdentityRecord &identity) const {
...
const std::unordered_map<int, int> &IdentityAssignmentEngineAdapter::Update(const std::vector<Track> &tracks,
  ...
  if (runtime_state_->occlusion_mode.mode == LegacyIdentityMode::kMerged && person_track_indices.size() == 1) {
    ...
    if (bootstrap_track_idx >= 0) {
      track_idx_to_sid[bootstrap_track_idx] = 1;
      sid_used[1] = true;
    }

    // continuity: keep existing raw->sid only when the current observation is still sane.
    for (std::size_t i = 0; i < person_track_indices.size(); ++i) {
      ...
      auto it = prev_raw_to_semantic.find(raw_id);
      if (it != prev_raw_to_semantic.end() && !sid_used[it->second]) {
        const int semantic_id = it->second;
        ...
        if (identity == nullptr) {
          ...
          continuity_input.passes_missing_identity_gate =
              PassesMissingIdentityGate(track, *identity, continuity_input.app_cost, continuity_input.geo_cost);
        }
        continuity_input.weak_mot_association = AssociationEvidenceWeakForIdentity(track.association);
        RawContinuityDecision::Config continuity_config;
        continuity_config.raw_continuity_max_cost = config_.raw_continuity_max_cost;
        const auto decision = RawContinuityDecision::Evaluate(continuity_input, continuity_config);
        runtime_state_->last_score_debug_rows.push_back(MakeRawContinuityDebugRow(runtime_state_->frame_index, runtime_state_->occlusion_mode.mode, decision));
    ...
    active_input.find_identity = [&](const int semantic_id) -> const LegacyIdentityRecord * {
    ...
    active_input.score_evidence = [&](const Track &track,
      ...
      ComputeCosts(track, identity, feature, &evidence.app_cost, &evidence.geo_cost, &evidence.time_cost,
                   &evidence.final_score);
      evidence.passes_missing_identity_gate =
          PassesMissingIdentityGate(track, identity, evidence.app_cost, evidence.geo_cost);
      evidence.passes_missing_appearance_gate =
          PassesMissingAppearanceGate(identity, evidence.app_cost, evidence.geo_cost);
      return evidence;
    };
    ...
    if (!active_input_result.builder_tracks.empty() && !active_input_result.builder_candidates.empty()) {
      ...
      const auto assigns = SolveAssignments(active_build, tracks);
      const bool phase4_pairwise_override_pending =
          std::any_of(runtime_state_->last_pairwise_assignment_debug_rows.begin(), runtime_state_->last_pairwise_assignment_debug_rows.end(),
                      [&](const auto &row) {
                        return row.frame_idx == runtime_state_->frame_index && row.appearance_override;
                      });
      ...
      for (const auto &row : active_rows) {
        for (auto &matcher_row : runtime_state_->last_score_debug_rows) {
          if (matcher_row.frame_idx == runtime_state_->frame_index && matcher_row.mode == runtime_state_->occlusion_mode.mode && matcher_row.track_idx == row.track_idx &&
              matcher_row.semantic_id == row.semantic_id && matcher_row.stage == row.stage) {
      ...
      for (const auto &a : assigns) {
        if (track_idx_to_sid.find(a.track_idx) != track_idx_to_sid.end()) {
          continue;
        }
        if (sid_used[a.semantic_id]) {
          continue;
        }
        if (!a.accepted || phase4_pairwise_override_pending) {
          continue;
        }
        track_idx_to_sid[a.track_idx] = a.semantic_id;
        sid_used[a.semantic_id] = true;
      }
    }
    ...
    recovery_input.find_identity = [&](const int semantic_id) -> const LegacyIdentityRecord * {
      return runtime_state_->identity_store.Find(semantic_id);
    };
    recovery_input.can_recover_identity = [&](const LegacyIdentityRecord &identity) {
      return CanRecoverInactiveIdentity(identity);
    };
    recovery_input.score_evidence =
        [&](const Track &track, const LegacyIdentityRecord &identity, const std::vector<float> &feature) {
          InactiveRecoveryInputCollector::ScoreEvidence evidence;
          ComputeCosts(track, identity, feature, &evidence.app_cost, &evidence.geo_cost, nullptr, nullptr);
          evidence.similarity = 1.0F - evidence.app_cost;
          evidence.recover_threshold = RecoverThresholdForSemantic(identity);
          evidence.passes_missing_identity_gate =
              PassesMissingIdentityGate(track, identity, evidence.app_cost, evidence.geo_cost);
          return evidence;
        };
    ...
    if (!recovery_input_result.solver_tracks.empty() && !recovery_input_result.solver_candidates.empty()) {
      ...
      AssignmentCandidateBuilder::ApplyInactiveRecoveryAssignments(recovery_result.assignments, &selected_recovery_rows);
    ...
    final_resolution_debug_rows.reserve(runtime_state_->last_score_debug_rows.size());
    ...
    final_resolution_input.frame_index = runtime_state_->frame_index;
    ...
    final_resolution_input.config.max_missing_frames = config_.max_missing_frames;
    final_resolution_input.config.defer_small_phase5_birth_allocation =
        config_.auto_apply_phase5_birth_allocations;
    final_resolution_input.find_identity = [&](const int semantic_id) -> const LegacyIdentityRecord * {
      return runtime_state_->identity_store.Find(semantic_id);
    };
    final_resolution_input.active_assignment_max_cost =
        [&](const LegacyIdentityRecord &identity, const AssociationEvidence &association) {
          return ActiveAssignmentMaxCost(identity, association);
        };
    ...
    final_resolution_input.looks_like_merged_side_reappearance =
        [&](const Track &candidate, const LegacyIdentityRecord &identity, const std::vector<Track> &all_tracks,
            const int candidate_idx, const std::unordered_map<int, int> &assigned_track_to_sid,
            const float app_cost) {
          return LooksLikeMergedSideReappearance(candidate, identity, all_tracks, candidate_idx,
                                                assigned_track_to_sid, app_cost);
        };
    final_resolution_input.evaluate_birth = [&](const BirthManager::Input &birth_input) {
      return runtime_state_->birth_manager.Evaluate(birth_input, [&]() { return AllocateNewSemanticId(); });
    };
    const auto final_resolution =
        UnresolvedTrackFinalResolutionCoordinator::Resolve(final_resolution_input);
    ...
    for (const auto &debug_row : final_resolution.debug_rows) {
      ...
      row.accepted = debug_row.accepted;
      row.reject_reason = debug_row.reject_reason;
      runtime_state_->last_score_debug_rows.push_back(std::move(row));
    }

    if (config_.auto_apply_phase5_birth_allocations) {
      IdentityRuntimeMutationApplier mutation_applier(config_, runtime_state_, &appearance_features_);
      ...
      for (const int raw_track_id : phase5_birth_allocation_raw_ids) {
        if (!mutation_applier.ApplyPhase5BirthAllocation(tracks, raw_track_id, frame)) {
          continue;
        }
        const int semantic_id = runtime_state_->raw_semantic_bindings.SemanticIdForRawTrack(raw_track_id);
        if (semantic_id <= 0) {
          continue;
        }
  ...
  // Do not hard-overwrite semantic IDs using primary raw track in NORMAL mode.
  // Keep semantic continuity from raw->semantic mapping and split recovery results.
  ...
  accepted_debug_rows.reserve(runtime_state_->last_score_debug_rows.size());
  for (const auto &row : runtime_state_->last_score_debug_rows) {
  ...
  const auto application_plan = AssignmentApplicationPlan::Build(
      application_candidates, accepted_debug_rows, runtime_state_->frame_index, occupied_semantic_ids, &runtime_state_->semantic_id_allocator,
      raw_map_track_order);
  ...
  AssignmentApplicationExecutor<ScoreDebugRow>::Execute(
      ...
      &runtime_state_->last_score_debug_rows, &runtime_state_->identity_store, &runtime_state_->raw_semantic_bindings, executor_config,
      [&](const Track &track, const int track_idx, const int semantic_id,
          const LegacyIdentityRecord *identity, const float assignment_cost,
          const float assignment_margin, const bool force_geometry_update) {
        (void)semantic_id;
        return EvaluateUpdatePolicy(track, tracks, track_idx, identity, true, assignment_cost,
                                    assignment_margin, force_geometry_update);
      },
