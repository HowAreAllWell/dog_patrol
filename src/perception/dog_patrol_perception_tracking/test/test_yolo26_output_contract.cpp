...
TEST(Yolo26OutputContractTest, AcceptsExpectedFinalDetectionShape) {
  vision_demo_host::Yolo26OutputContract contract;
  std::string error;

  ASSERT_TRUE(vision_demo_host::ValidateYolo26OutputShape({1, 300, 6}, &contract, &error)) << error;
...
TEST(Yolo26OutputContractTest, AcceptsAndRecordsNonStandardDetectionCount) {
  ...
  ASSERT_TRUE(vision_demo_host::ValidateYolo26OutputShape({1, 8400, 6}, &contract, &error)) << error;
...
TEST(Yolo26OutputContractTest, RejectsWrongRankWithContractMessage) {
  ...
  EXPECT_FALSE(vision_demo_host::ValidateYolo26OutputShape({300, 6}, nullptr, &error));
...
TEST(Yolo26OutputContractTest, RejectsWrongLastDimensionWithFieldMessage) {
  ...
  EXPECT_FALSE(vision_demo_host::ValidateYolo26OutputShape({1, 300, 85}, nullptr, &error));
...
TEST(Yolo26OutputContractTest, RejectsUnexpectedBatchDimension) {
  ...
  EXPECT_FALSE(vision_demo_host::ValidateYolo26OutputShape({2, 300, 6}, nullptr, &error));
...
TEST(Yolo26OutputContractTest, RejectsDynamicOrInvalidDetectionCount) {
  ...
  EXPECT_FALSE(vision_demo_host::ValidateYolo26OutputShape({1, -1, 6}, nullptr, &error));
