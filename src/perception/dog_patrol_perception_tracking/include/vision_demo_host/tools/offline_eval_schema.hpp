...
inline std::string Phase3ShadowStateCsvHelp() {
  return
      ...
      "  and event_type=new_birth_candidate_allocated for accepted phase5_new_semantic\n"
      "  allocation. Reasons include small_new_person_pending, small_stable_new_person_promoted,\n"
      "  phase5_birth_manager_allocated, ambiguous_recovery_pending, duplicate_split_hidden,\n"
      "  skinny_partial_hidden, and wide_fragment_hidden. candidate_semantic_id=-1 means no semantic id\n"
      "  was allocated; decision_* fields mirror the Phase 5 birth score row when present.\n"
      "  Hidden/pending/accepted birth decisions are expressed by the Phase 5 path behind\n"
      "  IdentityManager, with accepted allocation recorded as stage=phase5_new_semantic.\n"
      ...
      "  Accepted Phase 4 handoff paths are the only runtime behavior for merged/split handoff:\n"
      "  the migrated Phase 4 split handoff\n"
      "  emits event_type=phase4_merged_split_handoff with reason=merged_split_handoff.\n"
      "  The migrated Phase 4 side recovery\n"
      "  emits event_type=phase4_merged_side_recovery with reason=merged_side_recovery.\n"
      "  The migrated Phase 4\n"
      ...
      "  reason=side_reappearance_candidate when a side raw track is recovered by Phase 4 side recovery;\n"
      "  both rows link the candidate raw id\n"
