...
std::pair<double, int> SCManager::distanceBtnScanContext( MatrixXd &_sc1, MatrixXd &_sc2 )
{
    ...
    {
        shift_idx_search_space.push_back( (argmin_vkey_shift + ii + static_cast<int>(_sc1.cols())) % static_cast<int>(_sc1.cols()) );
        shift_idx_search_space.push_back( (argmin_vkey_shift - ii + static_cast<int>(_sc1.cols())) % static_cast<int>(_sc1.cols()) );
    }
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        ...
        ring_idx = std::max( std::min( PC_NUM_RING, static_cast<int>(ceil( (azim_range / PC_MAX_RADIUS) * PC_NUM_RING )) ), 1 );
        sctor_idx = std::max( std::min( PC_NUM_SECTOR, static_cast<int>(ceil( (azim_angle / 360.0) * PC_NUM_SECTOR )) ), 1 );
...
std::pair<int, float> SCManager::detectLoopClosureIDBetweenSession (std::vector<float>& _curr_key, Eigen::MatrixXd& _curr_desc)
{
    ...
    polarcontext_tree_batch_->index->findNeighbors( knnsearch_result, &curr_key[0] /* query */, nanoflann::SearchParams(10) ); 
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
     ...
     */
    if( static_cast<int>(polarcontext_invkeys_mat_.size()) < NUM_EXCLUDE_RECENT + 1)
    {
    ...
    else
    {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("scancontext"), 
                     "LiDAR ScanContext distance checking failed. Current: %.4f, Threshold: %.4f.", 
