...
public:
    ...
    void publish_odometry(
        const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr& pmavros_pose_publisherubOdomAftMapped);
    ...
    bool lidar_map_inited = false, pcd_save_en = false, pub_effect_point_en = false, pose_output_en = false,
    ...
    bool initial_dense_map_en_ = true;

    int img_en = 1, imu_int_frame = 3;
    int initial_img_en_ = 1;
    bool normal_en = true;
    ...
    // ArUco 前端检测 (仅在 img_en=1 时激活)
    rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr pubArucoDetections_;
#if CV_VERSION_MAJOR == 4 && CV_VERSION_MINOR >= 7
    ...
    double aruco_marker_size_ = 0.15;
    int aruco_kf_counter_ = 0;  // 用于给每次检测分配一个递增关键帧 ID
    ...
    // Dynamic tracking visual toggle
    bool disable_vision_on_tracking = false;
    bool tracking_locked_ = false;
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr sub_loc_status_;
    void loc_status_cbk(const std_msgs::msg::Bool::SharedPtr msg);

    // Rviz shadow members
    Eigen::Matrix4f T_body_to_baselink_ = Eigen::Matrix4f::Identity();
    double last_vio_time_ = 0.0;
};
#endif
