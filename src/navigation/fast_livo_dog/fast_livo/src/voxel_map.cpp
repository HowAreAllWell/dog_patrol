...
void VoxelMapManager::StateEstimation(StatesGroup& state_propagat) {
    ...
    for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++) {
        ...
        cout << "[ LIO ] Raw feature num: " << feats_undistort_->size()
             << ", downsampled feature num:" << feats_down_size_ << " effective feature num: " << effct_feat_num_
        ...
        for (int i = 0; i < effct_feat_num_; i++) {
            ...
            // point_w cov
            // var = state_propagat.rot_end * extR_ * ptpl_list_[i].body_cov_ * (state_propagat.rot_end *
            // extR_).transpose() +
            //       state_propagat.cov.block<3, 3>(3, 3) + (-point_crossmat) * state_propagat.cov.block<3, 3>(0, 0) *
            ...
            // point_body cov
            var =
                state_propagat.rot_end * extR_ * ptpl_list_[i].body_cov_ * (state_propagat.rot_end * extR_).transpose();
            ...
            Hsub.row(i) << VEC_FROM_ARRAY(A), ptpl_list_[i].normal_[0], ptpl_list_[i].normal_[1],
                ptpl_list_[i].normal_[2];
            Hsub_T_R_inv.col(i) << A[0] * R_inv(i), A[1] * R_inv(i), A[2] * R_inv(i),
                ptpl_list_[i].normal_[0] * R_inv(i), ptpl_list_[i].normal_[1] * R_inv(i),
        ...
        MD(DIM_STATE, DIM_STATE) && K_1 =
            (H_T_H.block<DIM_STATE, DIM_STATE>(0, 0) + state_.cov.block<DIM_STATE, DIM_STATE>(0, 0).inverse())
                .inverse();
        G.block<DIM_STATE, 6>(0, 0) = K_1.block<DIM_STATE, 6>(0, 0) * H_T_H.block<6, 6>(0, 0);
        
        // --- 探针: 打印 H_T_H 信息矩阵的最小特征值，监控退化爆炸 ---
        if (iterCount == 0) {
            Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> es_hth(H_T_H.block<6,6>(0,0));
            double min_eig_hth = es_hth.eigenvalues()(0);
            double max_eig_hth = es_hth.eigenvalues()(5);
            RCLCPP_WARN(rclcpp::get_logger("fast_livo_probe"),
                        "[Degeneracy Probe] H_T_H Min Eigen: %.4f | Max: %.4f | Condition Number: %.4f | Eff Feat: %d",
                        min_eig_hth, max_eig_hth, (max_eig_hth / (min_eig_hth + 1e-6)), effct_feat_num_);
        }
        // ----------------------------------------------------

        auto vec = state_propagat - state_;
        VD(DIM_STATE)
        solution = K_1.block<DIM_STATE, 6>(0, 0) * HTz + vec.block<DIM_STATE, 1>(0, 0) -
    ...
    // ave_build_residual_time = ave_build_residual_time * (scan_count - 1) / scan_count + build_residual_time /
    // scan_count; ave_ekf_time = ave_ekf_time * (scan_count - 1) / scan_count + ekf_time / scan_count;

    // cout << "[ Mapping ] ekf_time: " << ekf_time << "s, build_residual_time: " << build_residual_time << "s" << endl;
    // cout << "[ Mapping ] ave_ekf_time: " << ave_ekf_time << "s, ave_build_residual_time: " << ave_build_residual_time
    // << "s" << endl;
}

void VoxelMapManager::TransformLidar(const Eigen::Matrix3d rot, const Eigen::Vector3d t,
...
void VoxelMapManager::build_single_residual(pointWithVar& pv, const VoxelOctoTree* current_octo,
                                            const int current_layer, bool& is_sucess, double& prob,
                                            PointToPlane& single_ptpl) {
    int max_layer = config_setting_.max_layer_;
    double sigma_num = config_setting_.sigma_num_;
