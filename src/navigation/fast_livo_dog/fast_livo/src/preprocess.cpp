        ...
        for (uint i = 0; i < plsize; i++) {
            if ((msg->points[i].line < N_SCANS))  // && ((msg->points[i].tag & 0x30) == 0x10))
            {
                ...
                if (i == 0)
                    ...
                    // if(fabs(pl_full[i].curvature - pl_full[i - 1].curvature) > 1.0) ROS_ERROR("time jump: %f",
                    // fabs(pl_full[i].curvature - pl_full[i - 1].curvature));
                ...
                if (valid_num % point_filter_num == 0) {
                    if (pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z >=
                        blind_sqr) {
...
void Preprocess::Pandar128_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg) {
    ...
    auto comparePoints = [](const PointType& a, const PointType& b) -> bool { return a.curvature < b.curvature; };
...
bool Preprocess::edge_jump_judge(const PointCloudXYZI& pl, vector<orgtype>& types, uint i, Surround nor_dir) {
    if (nor_dir == 0) {
        if (types[i - 1].range < blind_sqr || types[i - 2].range < blind_sqr) {
            return false;
        }
    } else if (nor_dir == 1) {
        if (types[i + 1].range < blind_sqr || types[i + 2].range < blind_sqr) {
