...
LIVMapper::LIVMapper(rclcpp::Node::SharedPtr& node, std::string node_name, const rclcpp::NodeOptions& options)
    : node(std::make_shared<rclcpp::Node>(node_name, options)), extT(0, 0, 0), extR(M3D::Identity()) {
    ...
    if (!this->node->has_parameter("T_lidar_base")) {
        this->node->declare_parameter<std::vector<double>>("T_lidar_base", std::vector<double>(16, 0.0));
    }
    this->node->get_parameter("T_lidar_base", ext_matrix);
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr& node) {
    // declare parameters
    auto try_declare = [node]<typename ParameterT>(const std::string& name, const ParameterT& default_value) {
    ...
    try_declare.template operator()<bool>("common.disable_vision_on_tracking", false);
    ...
    try_declare.template operator()<vector<double>>("extrin_calib.T_imu_lidar", vector<double>(16, 0.0));
    try_declare.template operator()<vector<double>>("extrin_calib.T_camera_lidar", vector<double>(16, 0.0));
    ...
    try_declare.template operator()<int>("camera.width", 640);
    try_declare.template operator()<int>("camera.height", 512);
    ...
    this->node->get_parameter("common.disable_vision_on_tracking", disable_vision_on_tracking);
    ...
    vector<double> T_imu_lidar;
    vector<double> T_camera_lidar;
    this->node->get_parameter("extrin_calib.T_imu_lidar", T_imu_lidar);
    this->node->get_parameter("extrin_calib.T_camera_lidar", T_camera_lidar);

    if (T_imu_lidar.size() == 16) {
        extrinR = {T_imu_lidar[0], T_imu_lidar[1], T_imu_lidar[2], T_imu_lidar[4], T_imu_lidar[5],
                   T_imu_lidar[6], T_imu_lidar[8], T_imu_lidar[9], T_imu_lidar[10]};
        extrinT = {T_imu_lidar[3], T_imu_lidar[7], T_imu_lidar[11]};
    } else {
        RCLCPP_ERROR(this->node->get_logger(), "Invalid T_imu_lidar parameter size!");
    }

    if (T_camera_lidar.size() == 16) {
        cameraextrinR = {T_camera_lidar[0], T_camera_lidar[1], T_camera_lidar[2], T_camera_lidar[4], T_camera_lidar[5],
                         T_camera_lidar[6], T_camera_lidar[8], T_camera_lidar[9], T_camera_lidar[10]};
        cameraextrinT = {T_camera_lidar[3], T_camera_lidar[7], T_camera_lidar[11]};
    } else {
        RCLCPP_ERROR(this->node->get_logger(), "Invalid T_camera_lidar parameter size!");
    }
    ...
    initial_dense_map_en_ = dense_map_en;

    // ArUco 参数 (只尝试 get，不 declare，防止因为 yaml 中已存在而导致异常)
    if (this->node->has_parameter("aruco.aruco_en")) this->node->get_parameter("aruco.aruco_en", aruco_en_);
    if (this->node->has_parameter("aruco.marker_size")) this->node->get_parameter("aruco.marker_size", aruco_marker_size_);
    std::string aruco_dict_name = "DICT_6X6_50";
    if (this->node->has_parameter("aruco.dictionary")) this->node->get_parameter("aruco.dictionary", aruco_dict_name);
    
    // 自动补齐 DICT_ 前缀
    if (aruco_dict_name.find("DICT_") != 0) {
        aruco_dict_name = "DICT_" + aruco_dict_name;
    }
    ...
    if (aruco_en_) {
        auto it = dict_map.find(aruco_dict_name);
        if (it != dict_map.end()) {
#if CV_VERSION_MAJOR == 4 && CV_VERSION_MINOR >= 7
...
#endif
            if (img_en) {
                RCLCPP_INFO(this->node->get_logger(), "ArUco: Frontend enabled: dict=%s, size=%.2fm",
                    aruco_dict_name.c_str(), aruco_marker_size_);
            }
        } else {
            RCLCPP_WARN(this->node->get_logger(), 
                "ArUco: Invalid dictionary name '%s'. ArUco detection disabled.", aruco_dict_name.c_str());
            aruco_en_ = false;
        }
    }
...
void LIVMapper::initializeComponents(rclcpp::Node::SharedPtr& node) {
    ...
    initial_img_en_ = img_en;
    if (img_en == 1) {
        RCLCPP_WARN(this->node->get_logger(), ">>> [SLAM MAPPING/ODOM MODE] : LIVO (LiDAR + IMU + Vision)");
        RCLCPP_WARN(this->node->get_logger(), ">>> [FRONTEND ODOMETRY]      : LIVO Tight-Coupled Odometry");
    } else {
        RCLCPP_WARN(this->node->get_logger(), ">>> [SLAM MAPPING/ODOM MODE] : ONLY LIO (LiDAR + IMU)");
        RCLCPP_WARN(this->node->get_logger(), ">>> [FRONTEND ODOMETRY]      : Pure LIO Odometry (Vision Ignored)");
    }
}
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr& node,
    ...
    if (p_pre->lidar_type == AVIA) {
        sub_pcl = this->node->create_subscription<livox_ros_driver2::msg::CustomMsg>(
            lid_topic, 2000, std::bind(&LIVMapper::livox_pcl_cbk, this, std::placeholders::_1));
    } else {
        sub_pcl = this->node->create_subscription<sensor_msgs::msg::PointCloud2>(
            lid_topic, 2000, std::bind(&LIVMapper::standard_pcl_cbk, this, std::placeholders::_1));
    }
    sub_imu = this->node->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, 2000, std::bind(&LIVMapper::imu_cbk, this, std::placeholders::_1));
    sub_img = this->node->create_subscription<sensor_msgs::msg::Image>(
        img_topic, 2000, std::bind(&LIVMapper::img_cbk, this, std::placeholders::_1));
    sub_loc_status_ = this->node->create_subscription<std_msgs::msg::Bool>(
        "/localization_status", 10, std::bind(&LIVMapper::loc_status_cbk, this, std::placeholders::_1));
    ...
    mavros_pose_publisher =
        this->node->create_publisher<geometry_msgs::msg::PoseStamped>("/mavros/vision_pose/pose", 10);
    pubImage = it.advertise("/rgb_img", 1);
    // ArUco 检测结果发布器 (不放图像，放解析后的观测数据到 visual_loop_node)
    pubArucoDetections_ = this->node->create_publisher<std_msgs::msg::Float64MultiArray>("/aruco_detections", 10);
    pubImuPropOdom = this->node->create_publisher<nav_msgs::msg::Odometry>("/LIVO2/imu_propagate", 2000);
    ...
    voxelmap_manager->voxel_map_pub_ =
        this->node->create_publisher<visualization_msgs::msg::MarkerArray>("/planes", 2000);
}
...
void LIVMapper::processImu() {
    ...
    // std::cout << "[ Mapping ] predict sta: " << state_propagat.pos_end.transpose() <<
...
void LIVMapper::handleVIO() {
    ...
    fout_pre << std::setw(20) << LidarMeasures.last_lio_update_time - _first_lidar_time << " "
             << euler_cur.transpose() * 57.3 << " " << _state.pos_end.transpose() << " " << _state.vel_end.transpose()
    ...
    vio_manager->processFrame(LidarMeasures.measures.back().img, _pv_list, voxelmap_manager->voxel_map_,
    ...
    last_vio_time_ = this->node->now().seconds();

    euler_cur = RotMtoEuler(_state.rot_end);
    fout_out << std::setw(20) << LidarMeasures.last_lio_update_time - _first_lidar_time << " "
             << euler_cur.transpose() * 57.3 << " " << _state.pos_end.transpose() << " " << _state.vel_end.transpose()
             << " " << _state.bias_g.transpose() << " " << _state.bias_a.transpose() << " "
             << V3D(_state.inv_expo_time, 0, 0).transpose() << " " << feats_undistort->points.size() << std::endl;
}
...
void LIVMapper::handleLIO() {
    ...
    if (pose_output_en) {
        ...
        evoFile << LidarMeasures.last_lio_update_time << " " << _state.pos_end[0] << " " << _state.pos_end[1] << " "
                << _state.pos_end[2] << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << std::endl;
    }
    ...
    for (size_t i = 0; i < world_lidar->points.size(); i++) {
        voxelmap_manager->pv_list_[i].point_w << world_lidar->points[i].x, world_lidar->points[i].y,
    ...
    *pcl_w_wait_pub = *laserCloudWorld;
    ...
    // printf("[ mapping time ]: per scan: propagation %0.6f downsample: %0.6f match: %0.6f solve: %0.6f  ICP: %0.6f map
    // incre: %0.6f total: %0.6f \n"
    //         "[ mapping time ]: average: icp: %0.6f construct H: %0.6f, total: %0.6f \n",
    //         t_prop - t0, t1 - t_prop, match_time, solve_time, t3 - t1, t5 - t3, t5 - t0, aver_time_icp,
    //         aver_time_const_H_time, aver_time_consu);

    // printf("\033[1;36m[ LIO mapping time ]: current scan: icp: %0.6f secs, map incre: %0.6f secs, total: %0.6f
    // secs.\033[0m\n"
    //         "\033[1;36m[ LIO mapping time ]: average: icp: %0.6f secs, map incre: %0.6f secs, total: %0.6f
...
void LIVMapper::savePCD() {
    if (pcd_save_en && (pcl_wait_save->points.size() > 0 || pcl_wait_save_intensity->points.size() > 0) &&
        ...
        if (img_en) {
            pcl::PointCloud<PointTypeRGB>::Ptr downsampled_cloud(new pcl::PointCloud<PointTypeRGB>);
            ...
            pcd_writer.writeBinary(downsampled_points_dir,
            ...
            if (colmap_output_en) {
                ...
                for (size_t i = 0; i < downsampled_cloud->size(); ++i) {
                    ...
                    fout_points << i << " " << std::fixed << std::setprecision(6) << point.x << " " << point.y << " "
                                << point.z << " " << static_cast<int>(point.r) << " " << static_cast<int>(point.g)
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr msg_in) {
    ...
    // if ((abs(stamp2Sec(msg->header.stamp) - last_timestamp_lidar) > 0.2 && last_timestamp_lidar > 0) ||
    // sync_jump_flag)
    // {
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr msg_in) {
    ...
    if (last_timestamp_imu > 0.0 && timestamp < last_timestamp_imu) {
        RCLCPP_ERROR(this->node->get_logger(), "imu loop back, offset: %lf. Resetting imu timestamp.\n", last_timestamp_imu - timestamp);
        imu_buffer.clear();
        last_timestamp_imu = timestamp;
        mtx_buffer.unlock();
        sig_buffer.notify_all();
    ...
    if (last_timestamp_imu > 0.0 && timestamp > last_timestamp_imu + 0.2) {
        RCLCPP_WARN(this->node->get_logger(), "imu time stamp Jumps %0.4lf seconds. Resetting imu timestamp.\n", timestamp - last_timestamp_imu);
        imu_buffer.clear();
        last_timestamp_imu = timestamp;
        mtx_buffer.unlock();
        sig_buffer.notify_all();
...
cv::Mat LIVMapper::getImageFromMsg(const sensor_msgs::msg::Image::ConstSharedPtr& img_msg) {
    ...
    // 自动自适应分辨率：如果输入分辨率与配置不一致，自动缩放至 yaml 中声明的目标相机内参尺寸
    int target_width = (vio_manager && vio_manager->cam) ? vio_manager->width : 0;
    int target_height = (vio_manager && vio_manager->cam) ? vio_manager->height : 0;
    if (target_width <= 0 || target_height <= 0) {
        if (this->node->has_parameter("camera.width")) this->node->get_parameter("camera.width", target_width);
        if (this->node->has_parameter("camera.height")) this->node->get_parameter("camera.height", target_height);
    }
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr msg_in) {
    // 无论是相机端降采样还是算法自己降采样，如果 VIO 离线/静默 (或者完全禁用)，则在此发布降采样后的数据到 /rgb_img
    double current_time = this->node->now().seconds();
    if (!img_en || current_time - last_vio_time_ > 0.5) {
        // Limit publish rate to save CPU if img_en is false
        static double last_pub_time = 0;
        if (current_time - last_pub_time > 0.1) {
            last_pub_time = current_time;
            cv::Mat resized_img = getImageFromMsg(msg_in);
            if (!resized_img.empty()) {
                cv_bridge::CvImage out_msg;
                out_msg.header = msg_in->header;
                out_msg.encoding = sensor_msgs::image_encodings::BGR8;
                out_msg.image = resized_img;
    ...
    if (msg_header_time < last_timestamp_img) {
        RCLCPP_ERROR(this->node->get_logger(), "image loop back. Resetting image timestamp.\n");
        img_buffer.clear();
    ...
    switch (slam_mode_) {
        case ONLY_LIO: {
            ...
            if (!lidar_pushed) {
                ...
                meas.lidar_frame_end_time = meas.lidar_frame_beg_time + meas.lidar->points.back().curvature /
                                                                            double(1000);  // calc lidar scan end time
                meas.pcl_proc_cur = meas.lidar;
                lidar_pushed = true;  // flag
            }

            if (imu_en && last_timestamp_imu < meas.lidar_frame_end_time) {  // waiting imu message needs to be
...
void LIVMapper::publish_img_rgb(const image_transport::Publisher& pubImage, VIOManagerPtr vio_manager) {
    cv::Mat img_rgb = vio_manager->img_cp.clone();
    if (img_rgb.empty()) return;

    // ArUco 检测 (只在 img_en && aruco_en_ 时激活)
#if CV_VERSION_MAJOR == 4 && CV_VERSION_MINOR >= 7
    // In OpenCV 4.7+, Dictionary and DetectorParameters are value types, but aruco_en_ is true only if we found dict.
    // Actually, in 4.7+, just check aruco_en_ since they are not pointers.
    if (aruco_en_) {
#else
    if (aruco_en_ && aruco_dict_ && aruco_params_) {
#endif
        cv::Mat gray;
...
#if CV_VERSION_MAJOR == 4 && CV_VERSION_MINOR >= 7
        cv::aruco::ArucoDetector detector(aruco_dict_, aruco_params_);
        ...
        if (!ids.empty()) {
            RCLCPP_INFO(this->node->get_logger(), "ArUco: Detected %zu marker(s): ID=%d",
                ids.size(), ids[0]);
            // 绘制加粗的红色边框，默认 cv::aruco::drawDetectedMarkers 线条太细
            for (size_t i = 0; i < corners.size(); i++) {
                std::vector<cv::Point> pts;
                for (size_t j = 0; j < corners[i].size(); j++) {
                    pts.push_back(cv::Point((int)corners[i][j].x, (int)corners[i][j].y));
                }
                cv::polylines(img_rgb, pts, true, cv::Scalar(0, 0, 255), 3, cv::LINE_AA);
                
                // 标记 ID (左上角往外挪一点，防止和边框重叠)
                cv::Point text_pos(pts[0].x - 15, pts[0].y - 15);
                cv::putText(img_rgb, std::to_string(ids[i]), text_pos, 
                            cv::FONT_HERSHEY_SIMPLEX, 1.2, cv::Scalar(0, 0, 255), 3, cv::LINE_AA);
            }

            // 发布检测结果到 /aruco_detections
            // 格式: [kf_id, marker_id, tx, ty, tz, qx, qy, qz, qw, ...] 每个 marker 9 个 double
            // 暂时只发 marker_id 和 corner 中心像素坐标， PnP 先不在这里计算
            // 让 visual_loop_node 来做 PnP + 回环判断
            std_msgs::msg::Float64MultiArray det_msg;
            det_msg.data.push_back(static_cast<double>(aruco_kf_counter_++));
            det_msg.data.push_back(static_cast<double>(ids.size()));
            for (size_t i = 0; i < ids.size(); ++i) {
                det_msg.data.push_back(static_cast<double>(ids[i]));
                // 4 corners (u,v) * 4 = 8 doubles
                for (const auto& pt : corners[i]) {
                    det_msg.data.push_back(static_cast<double>(pt.x));
                    det_msg.data.push_back(static_cast<double>(pt.y));
                }
            }
            pubArucoDetections_->publish(det_msg);
        }
    }
...
void LIVMapper::publish_frame_world(
        ...
        *pcl_wait_pub += *pcl_w_wait_pub;
        if (pub_num == pub_scan_num) {
            ...
            for (size_t i = 0; i < size; i++) {
                ...
                pointRGB.intensity = pcl_wait_pub->points[i].intensity;
                pointRGB.curvature = pcl_wait_pub->points[i].curvature;
                pointRGB.normal_x = pcl_wait_pub->points[i].normal_x;
                pointRGB.normal_y = pcl_wait_pub->points[i].normal_y;
                pointRGB.normal_z = pcl_wait_pub->points[i].normal_z;
                ...
                if (vio_manager->new_frame_->cam_->isInFrame(pc.cast<int>(), 3))  // 100
                {
                    ...
                    float intensity = pcl_wait_pub->points[i].intensity;
                    int gray_val = std::min(255, std::max(0, static_cast<int>(intensity)));
                    pointRGB.r = gray_val;
                    pointRGB.g = gray_val;
                    pointRGB.b = gray_val;
                }
    ...
    if (pcd_save_en) {
        ...
        if ((pcl_wait_save->size() > 0 || pcl_wait_save_intensity->size() > 0) && pcd_save_interval > 0 &&
            ...
            if (pcd_save_en) {
                ...
                if (img_en) {
                    pcd_writer.writeBinary(
                        all_points_dir, *pcl_wait_save);  // pcl::io::savePCDFileASCII(all_points_dir, *pcl_wait_save);
...
void LIVMapper::publish_visual_sub_map(
    const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& pubSubVisualMap) {
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr& pubOdomAftMapped) {
    ...
    br->sendTransform(geometry_msgs::msg::TransformStamped(
        createTransformStamped(transform, odomAftMapped.header.stamp, "camera_init", "aft_mapped")));
    pubOdomAftMapped->publish(odomAftMapped);
}

void LIVMapper::publish_mavros(
    const rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr& mavros_pose_publisher) {
...
void LIVMapper::publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr& pubPath) {
    ...
    Eigen::Matrix4f T_camera_init_to_body = Eigen::Matrix4f::Identity();
    T_camera_init_to_body.block<3,3>(0,0) = _state.rot_end.cast<float>();
    T_camera_init_to_body.block<3,1>(0,3) = _state.pos_end.cast<float>();
    
    Eigen::Matrix4f T_camera_init_to_baselink = T_camera_init_to_body * T_body_to_baselink_;
}

void LIVMapper::loc_status_cbk(const std_msgs::msg::Bool::SharedPtr msg) {
    bool was_locked = tracking_locked_;
    tracking_locked_ = msg->data;
    if (tracking_locked_ && !was_locked) {
        if (disable_vision_on_tracking && initial_img_en_ == 1) {
            img_en = 0;
            dense_map_en = false;
            slam_mode_ = ONLY_LIO;
            RCLCPP_WARN(this->node->get_logger(), ">>> [RELOCALIZATION SUCCESS] : Global Pose LOCKED!");
            RCLCPP_WARN(this->node->get_logger(), ">>> [TRACKING MODE SWITCH]   : Switched from LIVO -> ONLY LIO (CPU Optimization)");
            RCLCPP_WARN(this->node->get_logger(), ">>> [CURRENT LOCAL TRACKING] : Pure LiDAR Geometry + IMU Tracking (Vision SLEEP)");
        } else if (img_en == 0) {
            RCLCPP_WARN(this->node->get_logger(), ">>> [RELOCALIZATION SUCCESS] : Global Pose LOCKED!");
            RCLCPP_WARN(this->node->get_logger(), ">>> [CURRENT LOCAL TRACKING] : Active Tracking in ONLY LIO Mode");
        } else {
            RCLCPP_WARN(this->node->get_logger(), ">>> [RELOCALIZATION SUCCESS] : Global Pose LOCKED!");
            RCLCPP_WARN(this->node->get_logger(), ">>> [CURRENT LOCAL TRACKING] : Active Tracking in LIVO Mode");
        }
    } else if (!tracking_locked_ && was_locked) {
        if (disable_vision_on_tracking && initial_img_en_ == 1) {
            img_en = 1;
            dense_map_en = initial_dense_map_en_;
            slam_mode_ = (lidar_en) ? LIVO : ONLY_LO;
            RCLCPP_WARN(this->node->get_logger(), ">>> [RELOCALIZATION SEARCH]  : Global Pose LOST or SEARCHING...");
            RCLCPP_WARN(this->node->get_logger(), ">>> [TRACKING MODE SWITCH]   : Switched back to LIVO (Active Vision Support)");
        } else {
            RCLCPP_WARN(this->node->get_logger(), ">>> [RELOCALIZATION SEARCH]  : Global Pose LOST or SEARCHING...");
        }
    }
