# worklog
## 2026-09-08 - 延长已处理目标的再次选择抑制时间

- 感知在目标任务结束并返回 `PATROL` 时，会记录上一目标的 semantic ID；该目标只有在连续不可见达到
  `target.handled_ignore_absence_sec` 后才重新具备巡检选择资格。目标持续可见时不会因为计时器到期而解除。
- 为避免恢复巡检转头后立即把刚认证/跟踪过的人重新确认，将该参数从 `30.0 s` 统一调整为 `60.0 s`。
- 该抑制只影响上一已处理目标，不阻止其他合格目标被选择；连续离开视野超过 60 秒后仍可再次选择。

## 2026-09-08 - 调整人脸质量门槛以支持当前采集距离

- 真机验证 `/home/orin/Pictures/Screenshots/face.png`：检测到 1 张人脸，置信度 `0.789`，与 `zby`
  白名单相似度 `0.9939`；原配置 `face_min_box_ratio=0.08` 因人脸框占图像 `0.0411` 而在特征匹配前拒绝。
- 将生产资产、face 包默认配置和 verifier 默认值统一调整为 `face_min_box_ratio=0.03`，保留相对当前
  实测比例约 27% 的余量，同时不降低到过于宽松的远距离误检范围。
...
- 修复移动 waypoint 时无条件清除 `sequence_done` 的问题。移动已完成点、未来点或当前
  未完成点都保持原有完成状态，不会改变其他 waypoint 的状态；只有当前正在执行的未完成
  waypoint 被移动时，才清理当前路径并触发重新规划。
- 统一普通删除和 `Undo` 的进度处理：删除已完成点只调整 `current_index` 以保持原来的
  逻辑目标，删除中间未完成点只让后续点按顺序接替当前目标，不重新打开已完成点。
- 修复删除最后一个未完成 waypoint 的边界：如果删除后剩余点都已经完成，保持
  `sequence_done=true`，不把最后一个已完成点重新显示为未完成。
- 增加 waypoint 完成进度、最后一个未完成点删除以及已完成/未来点编辑的回归测试；聚焦
  测试 9 项全部通过，Python 语法检查和 `git diff --check` 通过。
