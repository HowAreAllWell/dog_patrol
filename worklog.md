...
- 完成：新增 ROS-independent `PrimaryTargetObserver` / `PrimaryTargetObservation` / `PrimaryTargetObservationSink`，输出 semantic target ID、源帧元数据、clamped 可信 bbox、confidence 和深拷贝目标图；standalone live 使用线程安全 latest sink 消费每帧结果，无当前可信目标时显式清空。live 节点新增统一 runtime strategy 封装强类型 `mission|standalone` 模式各自的初始化失败策略、status、before-frame、current-primary 和 process-frame 行为；standalone 不创建 `MissionRosAdapter`，复用原 camera/detector/tracker/identity/primary/visualizer 链路；新增独立 launch、preview/record 开关、部署参数入口和 Orin 相机说明；portable CI 增加 live node translation-unit 编译检查；Standards/Spec 双轴审查发现的分支散落、输出 data clump、可信判定重复、observation 未消费和 semantic ID 回退问题均已修复；mission integration fixture 在发布一次性 capability 前同时等待真实 readiness subscriber，消除全套顺序运行的 DDS discovery 竞态。
