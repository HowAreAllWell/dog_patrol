...
- 验证：五包 portable 构建通过；全仓汇总 414 tests、0 errors/failures/skipped；其中 tracking 50/50 CTest 通过，覆盖 crop 消息映射、外扩与边界裁剪、generation 失效取消、慢 publish worker 非阻塞和故意变慢的独立 DDS 消费者/快速 observer frame-loop smoke（连续 3 次通过）；Python 生成类型导入及无 `state_seq` 断言、`git diff --check` 通过。
